...
"""
Train a YOLOv5 model on a custom images. Models and datasets download automatically from the latest YOLOv5 release.
...
def train(hyp, opt, device, callbacks):
    """
    Train a YOLOv5 model on a custom images using specified hyperparameters, options, and device, managing datasets,
    ...
    if RANK in {-1, 0}:
        ...
        # Process custom images artifact link
    ...
    is_coco = isinstance(val_path, str) and val_path.endswith("coco/val2017.txt")  # COCO images
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        # images.mosaic_border = [b - imgsz, -b]  # height, width borders
...
def parse_opt(known=False):
    ...
    parser.add_argument("--artifact_alias", type=str, default="latest", help="Version of images artifact to use")
...
def main(opt, callbacks=Callbacks()):
    ...
    if opt.resume and not check_comet_resume(opt) and not opt.evolve:
        ...
        opt_data = opt.data  # original images
        if opt_yaml.is_file():
...
def run(**kwargs):
    ...
    Args:
        ...
        data (str, optional): Path to images YAML configuration. Defaults to ROOT / 'data/coco128.yaml'.
