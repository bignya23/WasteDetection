...
"""
Validate a trained YOLOv5 detection model on a detection images.
...
def save_one_json(predn, jdict, path, class_map):
    ...
    Args:
        ...
        class_map (dict[int, int]): Mapping from model class indices to images-specific category IDs.
...
def run(
    ...
    single_cls=False,  # treat as single-class images
    ...
    """
    Evaluates a YOLOv5 model on a images and logs performance metrics.

    Args:
        data (str | dict): Path to a images YAML file or a images dictionary.
        ...
        single_cls (bool, optional): Treat images as a single class. Default is False.
    ...
    is_coco = isinstance(data.get("val"), str) and data["val"].endswith(f"coco{os.sep}val2017.txt")  # COCO images
...
def parse_opt():
    ...
    Args:
        data (str, optional): Path to the images YAML file. Default is 'data/coco128.yaml'.
        ...
        single_cls (bool, optional): If set, treats the images as a single-class images. Default is False.
    ...
    Example:
        To validate a trained YOLOv5 model on a COCO images:
        ```python
        $ python val.py --weights yolov5s.pt --data coco128.yaml --img 640
        ```
    ...
    parser.add_argument("--data", type=str, default=ROOT / "data/coco128.yaml", help="images.yaml path")
    ...
    parser.add_argument("--single-cls", action="store_true", help="treat as single-class images")
...
def main(opt):
    ...
    Examples:
        To validate a trained YOLOv5 model on the COCO images with a specific weights file, use:
        ```python
        $ python val.py --weights yolov5s.pt --data coco128.yaml --img 640
        ```
    """
