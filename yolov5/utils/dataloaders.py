...
def create_dataloader(
    ...
    if rect and shuffle:
        ...
        shuffle = False
    with torch_distributed_zero_first(rank):  # init images *.cache only once if DDP
...
class _RepeatSampler:
    ...
    def __iter__(self):
        """Returns an infinite iterator over the images by repeatedly yielding from the given sampler."""
        while True:
...
class LoadImages:
    ...
    def __next__(self):
        """Advances to the next file in the images, raising StopIteration if at the end."""
    ...
    def __len__(self):
        """Returns the number of files in the images."""
...
class LoadStreams:
    ...
    def __iter__(self):
        """Resets and returns the iterator for iterating over video frames or images in a images."""
        self.count = -1
        return self
    ...
    def __len__(self):
        """Returns the number of sources in the images, supporting up to 32 streams at 30 FPS over 30 years."""
...
class LoadImagesAndLabels(Dataset):
    ...
    cache_version = 0.6  # images labels *.cache version
    ...
    def __init__(
        ...
        """Initializes the YOLOv5 images loader, handling images and their labels, caching, and preprocessing."""
        self.img_size = img_size
        ...
        if min_items:
            ...
            LOGGER.info(f"{prefix}{n - len(include)}/{n} images filtered from images")
    ...
    def check_cache_ram(self, safety_margin=0.1, prefix=""):
        ...
        for _ in range(n):
            ...
            b += im.nbytes * ratio**2
        mem_required = b * self.n / n  # GB required to cache images into RAM
    ...
    def cache_labels(self, path=Path("./labels.cache"), prefix=""):
        """Caches images labels, verifies images, reads shapes, and tracks images integrity."""
        x = {}  # dict
    ...
    def __len__(self):
        """Returns the number of images in the images."""
        return len(self.im_files)

    # def __iter__(self):
    #     self.count = -1
    #     print('ran images iter')
    #     #self.shuffled_vector = np.random.permutation(self.nF) if self.augment else np.arange(self.nF)
    #     return self

    def __getitem__(self, index):
        """Fetches the images item at the given index, considering linear, shuffled, or weighted sampling."""
...
def extract_boxes(path=DATASETS_DIR / "coco128"):
    """
    Converts a detection images to a classification images, creating a directory for each class and extracting
    bounding boxes.
...
def autosplit(path=DATASETS_DIR / "coco128/images", weights=(0.9, 0.1, 0.0), annotated_only=False):
    """Autosplit a images into train/val/test splits and save path/autosplit_*.txt files
...
class HUBDatasetStats:
    """
    Class for generating HUB images JSON and `-hub` images directory.

    Arguments:
        path:           Path to data.yaml or data.zip (with data.yaml inside data.zip)
        autodownload:   Attempt to download images if not found locally
    ...
    def __init__(self, path="coco128.yaml", autodownload=False):
        """Initializes HUBDatasetStats with optional auto-download for datasets, given a path to images YAML or ZIP
        file.
        """
        ...
        check_dataset(data, autodownload)  # download images if missing
    ...
    def _unzip(self, path):
        ...
        dir = path.with_suffix("")  # images directory == zip name
    ...
    def _hub_ops(self, f, max_dim=1920):
        ...
        f_new = self.im_dir / Path(f).name  # images-hub image filename
        try:  # use PIL
            im = Image.open(f)
    ...
    def get_json(self, save=False, verbose=False):
        """Generates images JSON for Ultralytics HUB, optionally saves or prints it; save=bool, verbose=bool."""
        ...
        for split in "train", "val", "test":
            if self.data.get(split) is None:
                ...
                continue
            dataset = LoadImagesAndLabels(self.data[split])  # load images
            x = np.array(
                [
